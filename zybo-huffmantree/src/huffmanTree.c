...
/// <summary>
/// Converts int number to char (maximum 999)
/// </summary>
/// <param name="output"></param>
char* intToStr(int intNum){
	char* ret = calloc(4,sizeof(char));
	int index=0;

//	*(ret + index++) = (char)0x30 + (intNum/1000);
	if(intNum/100){
		*(ret + index++) = (char)0x30 + (intNum/100);
		intNum = intNum - ((intNum/100)*100);
	}
	if(intNum/10){
		*(ret + index++) = (char)0x30 + (intNum/10);
		intNum = intNum - ((intNum/10)*10);
	}
	*(ret + index++) = (char)0x30 + (intNum/1);
	*(ret + index++) = (char)NULL;
...
//TODO; replace sprintf_s function

	conversion = intToStr(currentNode->priority);

	*(str + (i++)) = '(';
	*(str + (i++)) = currentNode->character;
	*(str + (i++)) = ',';
